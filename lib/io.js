...
class IO {
  constructor (socket, id, ctx, keyRotation) {
    this.id = id
    this.socket = socket
    this.inflight = []

    this._ctx = ctx
    this._rid = (Math.random() * MAX_REQUESTS) | 0
    this._requests = new Array(MAX_REQUESTS)
    this._pending = []
    ...
    this._rotateInterval = setInterval(this._onrotate.bind(this), keyRotation)
  ...
  _free () {
    const rid = this._rid++
    if (this._rid === MAX_REQUESTS) this._rid = 0
    return rid
  }
