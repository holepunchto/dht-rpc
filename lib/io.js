...
module.exports = class IO {
  constructor (network, table, udx, { maxWindow = 80, port = 0, host = '0.0.0.0', anyPort = true, firewalled = true, onrequest, onresponse = noop, ontimeout = noop } = {}) {
  ...
  onmessage (socket, buffer, { host, port }) {
    ...
    if (buffer[0] === REQUEST_ID) {
      const req = Request.decode(this, socket, from, this.network, state)
      if (req === null) return
    ...
    if (buffer[0] === RESPONSE_ID) {
      const res = decodeReply(from, state, this.network)
      if (res === null) return
...
class Request {
  ...
  static decode (io, socket, from, network, state) {
    try {
      ...
      if (id !== null) from.id = validateId(id, from, network)
...
function decodeReply (from, state, network) {
  const flags = c.uint.decode(state)
  const tid = c.uint16.decode(state)
  const to = peer.ipv4.decode(state)
  const id = flags & 1 ? c.fixed32.decode(state) : null
...
function validateId (id, from, network) {
  return b4a.equals(peer.id(from.host, from.port, network, TMP), id) ? id : null
}
