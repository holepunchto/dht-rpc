...
module.exports = class IO {
  constructor (table, udx, { maxWindow = 80, port = 0, host = '0.0.0.0', anyPort = true, firewalled = true, onrequest, onresponse = noop, ontimeout = noop } = {}) {
    ...
    this._port = port // can be a range ([startI, endI]) or an int
  ...
  async _bindSockets () {
    ...
    const isRange = this._port.length !== undefined

    const maxAttempts = isRange && !this._boundServerPort
      ? this._port[1] - this._port[0]
      : 1

    const initPort = isRange ? this._port[0] : this._port

    // TODO: expected behaviour when serversocket.bound is already true?
    // (like when calling bind multiple times).
    // To get the previous behaviour, we need to special-case the first iteration
    // so it always runs
    for (let i = 0; !serverSocket.bound && i < maxAttempts; i++) {
      try {
        serverSocket.bind(this._boundServerPort || initPort + i, this._host)
      } catch (err) {
        if (!this._anyPort) {
