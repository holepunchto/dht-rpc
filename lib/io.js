...
module.exports = class IO {
  constructor(
    ...
    this._internalCommandsSentPerTick = [0, 0, 0, 0]
    this._internalCommandsRateLimit = [-1, -1, -1, -1]
      ...
      for (const key in internalCommandsRateLimit) {
        switch (key) {
          ...
          case 'findNode':
            ...
            break
          case 'downHint':
            this._internalCommandsRateLimit[3] = internalCommandsRateLimit[key]
            break
          default:
  ...
  createRequest(to, token, internal, command, target, value, session, ttl) {
    ...
    if (internal && command < this._internalCommandsSentPerTick.length) {
