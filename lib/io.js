...
module.exports = class IO {
  ...
  async _destroy() {
    ...
    // A failed bind must not prevent cleanup, _clear tolerates missing sockets.
    try {
      await this.bind()
    } catch {}
  ...
  async _clear(suspended) {
    ...
    const closing = []
    if (this.serverSocket) closing.push(this.serverSocket.close())
    if (this.clientSocket) closing.push(this.clientSocket.close())
  ...
  async _rebind(binding) {
    try {
      // The previous binding may have failed. That is fine, we are binding again.
      try {
        if (binding) await binding
      } catch {}
      // destroy() awaits this promise, so returning _destroying here would deadlock
      if (this._destroying) return
      await this._bindSockets()
      this.networkInterfaces = this.udx.watchNetworkInterfaces()
      // Only accept traffic once we actually have sockets again
      this.suspended = false
    } catch (err) {
      // Do not leave a rejected promise behind, so the next bind()/resume() retries
      this._binding = null
      throw err
    } finally {
      this._rebinding = null
  ...
  resume() {
    if (!this.suspended) return this.bind()
    // A resume is already in flight, join it instead of binding twice and leaking sockets
    if (this._rebinding) return this._rebinding
    this._rebinding = this._rebind(this._binding)
    this._binding = this._rebinding
