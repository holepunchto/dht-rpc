...
module.exports = class IO {
  ...
  onmessage(socket, buffer, { host, port }) {
    ...
    if (buffer[0] === RESPONSE_ID) {
      ...
      for (let i = 0; i < this.inflight.length; i++) {
        ...
        if (this._peerRtt && req.sent === 1) {
          this._recordRtt(req.to, res.rtt)
        }
  ...
  _recordRtt(from, rtt) {
    ...
    let p = this._peerRtt.get(key)

    // rolling weighted averages
    if (!p) {
      // start with seen rtt and half the variability
      p = { srtt: rtt, rttvar: rtt >> 1 }
      this._peerRtt.set(key, p)
    } else {
      // blend 75% old + 25% new deviation
      p.rttvar += (Math.abs(p.srtt - rtt) - p.rttvar) >> 2
...
class Request {
  ...
  _sendNow() {
    ...
    this._timeout = setTimeout(oncycle, this._io._peerRtt ? this._getSendTimeout() : 1000, this)
  }

  _getSendTimeout() {
    const key = `${this.to.host}:${this.to.port}`
    const p = this._io._peerRtt.get(key)

    //  if peer always times out, we won't have p
    if (p) {
      // Known peer - use adaptive
      const baseRto = p.srtt + (p.rttvar << 1)
      const backoff = baseRto * this.sent
      const jitter = (Math.random() * 256) | 0
      return Math.min(Math.max(backoff + jitter, 300), 5000)
    } else {
      // Unknown peer - be aggressive
      const aggressive = [500, 750, 1000, 1500, 2000]
      const base = aggressive[Math.min(this.sent - 1, aggressive.length - 1)]
      return base + ((Math.random() * 256) | 0)
    }
  }
